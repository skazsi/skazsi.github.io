...
I will use an example from the real world since people can think with physical objects more easily than theoretical ones. Let's have an automobile factory where we focus on two aspects only.
...
We are switching our car manufacturing example to a trading application we recently started working on. The application provides only the user interface for the trading backend system. Hence, we need to send all transactions through the network. We want to avoid any integrity issues during the transmission, so we add a hash value as a checksum to each trading request. So far, so good.
